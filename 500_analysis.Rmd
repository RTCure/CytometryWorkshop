...
list(
    ...
    tar_target(live_model_total,
               Count ~ PatientType * Timepoint + (1|Individual) + offset(logTotal)
               ),
    tar_target(params_live_contrasts,{
        list("(Treated-Baseline)PT" = c(0,-1,0,1))
    }),
    ...
    mutate(model = map(data,
                       ~ tryCatch(
                           glmer.nb(live_model_total,
                                  data=.,
                                  control = glmerControl(optCtrl = 
                                                             list(maxfun = 25000))),
                       error = function(e){})),
           isError = map_lgl(model,is.null)) %>% 
    dplyr::filter(!isError) %>% 
    mutate(estimate = map2(model,
                           data,
                           ~ emmeans(.x,
