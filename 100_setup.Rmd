...
We need to define a handful of parameters first:
...
You can add parameters as you go about, such as `params_problem_markers`, to capture problematic antibodies.

As a rule of thumb, any marker that will help distinguish a population is *phenotypic*.
...
list(tar_target(params_path,
     ...
     tar_target(params_pheno_markers,
                c('CD3','CD19','CD45RA','CD8a','CD11c','CD16','CD34','CD14',
                  'TCRgd','CD7','CD33','CD20','CD123','CD56','CD45RO','CD25',
                  'CD4','CD11b')),
     tar_target(params_func_markers,
                c('CD69','CD31','CD86','TIGIT','CD27','CD29','CD366','Foxp3',
                  'CD38','CD127','CD68','CD62L','CD57','CD44','HLA-DR',
                  'CD279')),
     tar_target(params_problem_markers,
                c('CD163')),
     tar_target(params_annots_file,
                "input/HELIOS share/legend.xlsx",
                format = "file")
)
```
...
.annots <- readxl::read_xlsx(params_annots_file) %>% 
    dplyr::filter(!is.na(`name file`)) %>% 
    rename(FCSname = `name file`,
           Individual = `#patients`,
           Timepoint = `time of treatment`,
           BatchId = `acquisition date`,
           StainingId = `acquisition staining`) %>% 
    mutate(Individual = factor(Individual),
           Individual = relevel(Individual,
                                "Healty Donors"),
           Individual = fct_recode(Individual,
                                   "HD" = "Healty Donors"),
           PatientType = if_else(Individual=="HD",
                                 "HD","PT"),
           ...
           Timepoint = fct_recode(Timepoint,
                                  baseline = "n/a",
                                  treated = "treated (week 36)"),
           Timepoint = relevel(Timepoint,
                               "baseline"))
as_tibble(pData(.gs),
          rownames = "id") %>% 
    rename(FCSname = name,
...
as_tibble(pData(parameters(gs_cyto_data(.gs)[[1]])),
    ...
    mutate(desc = if_else(is.na(desc),
           ...
           type = case_when(
               ...
               desc %in% params_problem_markers ~ "problem",
               TRUE ~ "other"),
           desc = make.names(desc),
           type = factor(type,
                         levels = c("reference","phenotypic","functional",
                                    "problem","other")))
```
...
tar_read(live.df) %>%
    ...
    geom_vline(data = . %>% 
                   group_by(Channel) %>% 
                   reframe(value = quantile(value,c(0,0.98))),
